...
def parcel_info_single(parcel,data,t,num): #info for single parcels at single time
    ...
    u=parcel['u'].isel(time=t,xh=num).values/1000;
    v=parcel['v'].isel(time=t,xh=num).values/1000;
    w=parcel['w'].isel(time=t,xh=num).values/1000;
    ...
    #--------------Saves the variables 
    #parcel_num, whichx, whichy, whichz, whichlfc, whichlcl
    pinfo_s=np.zeros((1,10+1),dtype='object') #parcel_num,x,y,z, LFC, LCL, u, v, w
    pinfo_s[0,0]=num; pinfo_s[0,1]=which_x; pinfo_s[0,2]=which_y; pinfo_s[0,3]=z; pinfo_s[0,4]=lfc
    pinfo_s[0,5]=lcl; pinfo_s[0,6]=u; pinfo_s[0,7]=v; pinfo_s[0,8]=w; pinfo_s[0,9]=which_z; pinfo_s[0,10]=x #marks tracer number,which_x,which_y,z,lfc,lcl,u,v, and w at individual columns
    return(pinfo_s)
        ...
        if (pinfo[ind,4] <= pinfo[ind,3] <= pinfo[ind,4] + 1000/1000) and pinfo[ind,4]>0: #if above LFC (LFC must have a value)
            ...
            if np.sum(ascend_array >= ascend_thresh)/len(ascend_array) >= ascend_percent:
                ...
                for back_t in np.arange(t-1, starttime-1, -1): #counts from current time down to starttime 
                    ...
                    ###################################################################################################################################
                    if pinfo_s[0,10]+pinfo_s[0,6]>np.max(data['xf'].values) or pinfo_s[0,3]+pinfo_s[0,8]>np.max(data['zf'].values): #x is radiative, z non-periodic, forget parcel
                    ...
                    if pinfo_s[0,8]<w_thresh: 
                        ...
                        #6--------------if parcel below 750m (or LCL), check data for max convergence within 2km
                            ...
                            ###################################################################################################################################
                            maxconv_x=whereCL.isel(time=back_t,z=pinfo_s[0,9],y=pinfo_s[0,2])['maxconv_x'].values; #(z level must be < 5)
